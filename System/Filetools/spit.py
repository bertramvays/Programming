...
def split(fromfile, todir, chunksize=chunksize):
    ...
    partnum = 0
    input = open(fromfile, 'rb')  # двоичный режим: без декодирования и буз преобразования
    ...
    input.close()
    assert partnum <= 9999  # сортироквка в join не возможна, если будет 5 цифр
    return partnum
...
if __name__ == '__main__':
    if len(sys.argv) == 2 and sys.argv[1] == '-help':
        ...
        if interactive: input('Press Enter key')  # пауза если сценарий запущен
                                                  # щелчком мыши
# /home/bertramvays/Downloads/PythonNotesForProfessionals.pdf
