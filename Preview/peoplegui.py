...
храняшихся в хранилище;
хранилище находиться на том же компьютере, где выполняется сценарий в виде одного
...
def fetchRecord():
    key = entries['key'].get()
    try:
        record = db[key]  # извлечь запись по ключуб отобразить в форме
    except:
...
def updateRecord():
    key = entries['key'].get()
    if key in db:
        record = db[key]  # изменяется текущая запись
    else:
